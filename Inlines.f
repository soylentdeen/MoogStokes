...
333   if (linfileopt .eq. 0) then
     ...
     .                             dampnum(j),d0(j),width(j)
         ...
         c4(j) = 0.0
      elseif (linfileopt .eq. 2) then
         read (nfslines,1016,end=311) wave1(j),atom1(j),e(j,1),
     .            gf(j),dampnum(j),d0(j),width(j),deltamj(j),
     .            crad(j), c4(j)
      endif
      iatom = atom1(j)
...
1016  format (10e10.3)
      ...
      subroutine inlinesStokes (num)
c******************************************************************************
c     This subroutine reads in the line data -- Modified for MoogStokes!
c******************************************************************************
...
c*****output information about the isotopic ratios
      if (numiso .gt. 0) then
         write (nf1out,1014) 'Isotopic Ratios given for this synthesis'
         do i=1,numiso
            iiso = isotope(i)
            write (nf1out,1015) iiso, isotope(i), isoabund(i,isorun)
            if (nf2out .gt. 0) write (nf2out,1015) 
